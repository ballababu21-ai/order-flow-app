...
with tab1:
  ...
  for i in range(3):
    ...
    state_text = np.random.choice(
        ["STRONG ALIGNMENT", "FLOW ONLY | No wall touch", "MOMENTUM SPIKE"]
    )
  ...
  if not TF_AVAILABLE:
    ...
    if st.button("🚀 Train & Predict via Live LSTM Model"):
      with st.spinner(
        ...
        st.success(
            "✅ LSTM న్యూరల్ నెట్‌వర్క్ మోడల్ విజయవంతంగా శిక్షణ పొందింది!"
        )
        ...
        if price_diff > 0:
          st.success(
              "🟢 **AI Signal:** మోడల్ ప్రకారం రాబోయే కాలులో అప్‌సైడ్ మొమెంటం"
              " కనిపించే అవకాశం ఉంది."
          )
        else:
          st.error(
              "🔴 **AI Signal:** మోడల్ ప్రకారం షార్ట్ టర్మ్ లో డౌన్‌సైడ్ ప్రెషర్"
              " ఉండవచ్చు."
          )
