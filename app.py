...
with c1:
    st.markdown("""
        <div class="metric-box">
            ...
            <span style="background-color: #00C853; color: white; padding: 4px 12px; border-radius: 5px; font-weight: bold;">HIGH CONVICTION LONG ENTRY</span>
        </div>
    """, unsafe_allow_html=True)
...
with c2:
    st.markdown(f"""
        <div class="metric-box">
            <p style="color: #8B949E; margin:0;">CURRENT SPOT PRICE</p>
...
col_a.metric("DYNAMIC CALL WALL (RESISTANCE)", f"₹{market['call_wall']:,.2f}")
col_b.metric("DYNAMIC PUT WALL (SUPPORT)", f"₹{market['put_wall']:,.2f}")
col_c.metric("PUT-CALL RATIO (PCR)", f"{market['pcr']}")
